...
describe('skopeo module tests', () => {
  test.concurrent('getCredentialParameters()', async () => {
    ...
    const credentialParametersForNoCredentials = skopeo.getCredentialParameters(
      noCredentials,
    );
    expect(credentialParametersForNoCredentials).toEqual([]);

    const emptyCredentials = '';
    const credentialParametersForEmptyCredentials = skopeo.getCredentialParameters(
      emptyCredentials,
    );
    expect(credentialParametersForEmptyCredentials).toEqual([]);

    const someCredentials = 'secret-things-happening';
    const credentialParametersForSomeCredentials = skopeo.getCredentialParameters(
      someCredentials,
    );
