...
tap.test('getImagesWithFileSystemPath()', async (t) => {
  ...
  const expectedPattern =
    fileSystemPath.indexOf(`${config.IMAGE_STORAGE_ROOT}/nginx_latest_`) !== -1;
...
tap.test('extracted image tag tests', async (t) => {
  ...
  t.same(
    imageWithShaResult.imageDigest,
    'sha256:45b23dee08af5e43a7fea6c4cf9c25ccf269ee113168c19722f87876677c5cb2',
    'image digest is returned',
  );
  ...
  t.same(
    imageWithFullRepositoryResult.imageTag,
    'latest',
    'image tag is returned when full repo specified',
  );

  const imageWithoutTag = 'nginx';
  const imageWithoutTagResult = scannerImages.getImageParts(imageWithoutTag);
  t.same(
    imageWithoutTagResult.imageTag,
    '',
    'empty tag returned when no tag is specified',
  );
  ...
  t.same(
    imageWithManySeparatorsResult.imageTag,
    '',
    'empty tag is returned on malformed image name and tag',
  );
  ...
  t.same(
    imageWithFullRepoAndManySeparatorsResult.imageTag,
    '',
    'empty tag is returned on malformed image name and tag with full repo',
  );
});

tap.test('extracted image name tests', async (t) => {
  t.same(
    scannerImages.getImageParts('nginx:latest').imageName,
    'nginx',
    'removed image:tag',
  );
  t.same(
    ...
    'node',
    'removed image@sha:hex',
  );
  t.same(
    scannerImages.getImageParts('kind-registry:5000/python:rc-buster')
      .imageName,
    'kind-registry:5000/python',
    'removed repository/image:tag',
  );
  // Verify support on image names that contain dashes
  t.same(
    scannerImages.getImageParts('kind-registry:5000/python-27:rc-buster')
      .imageName,
    'kind-registry:5000/python-27',
    'removed repository/image:tag',
  );
  t.same(
    scannerImages.getImageParts('kind-registry:5000/test/python-27:rc-buster')
      .imageName,
    'kind-registry:5000/test/python-27',
    'removed repository/image:tag',
  );
});
